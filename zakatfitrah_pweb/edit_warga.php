...
<div class="content">
    ...
    <div>
        <p>Edit informasi Warga :</p>
        <table border="1">
            <?php
            $id = $_GET['id_warga'];
            $data = mysqli_query($con,"select * from warga where id_warga = '$id'");
            while($d = mysqli_fetch_array($data)){
                ?>
                <form method="post" action="update_warga.php">
                    <table>
                        <tr>
                            <td>ID</td>
                            <td><input type="text" name="id_warga" value="<?php echo $d['id_warga']; ?>"></td>
                        </tr>
                        <tr>
                            <td>Nama</td>
                            <td>
